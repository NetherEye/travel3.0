...
class Fetch {
	...
	importE(url, file, progressCallback, callback) {
		...
		param.append('file', file); //通过append向form对象添加数据
		var _headers = {'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8', 'token': this.getToken()}
		axios({
				method: 'post',
				url: url,
				data: param,
...
function checkStatusAndCode(response, callback, type){
	if(type == 'export'){
		...
		if(response['headers'] && response['headers']['content-disposition']){
			var realFileName = decodeURI(response['headers']['content-disposition'].split('filename=')[1]);
		}
...
//			type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8'
		...
		return callback({
			...
			href: href,
			data: response.data,
			realFileName: realFileName || 'xxx.xls'
		})
	}
