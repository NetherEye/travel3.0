		...
		<div class="box">
			...
			<div class="content">
				<div class="item" style="margin-bottom: 34px;">
					<div class="item_item">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>项目名称：</p>
						<p>
							<el-input v-model="form.projectName" placeholder="项目名称" :maxLength="50" clearable :disabled="type !='添加'&& type!='update'">
							</el-input>
						</p>
					</div>
					<div class="item_item">
						...
						<p>
							<el-input v-model="form.projectSubject" placeholder="项目主体" :maxLength="50" clearable :disabled="type !='添加'&& type!='update'">
							</el-input>
						</p>
					</div>
					<div class="item_item">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>项目地址：</p>
						<p>
							<template>
								<el-select v-model="form.projectAddress" style="width:100%" placeholder="全部" :disabled="type !='添加'&& type!='update'">
					...
					<div class="item_item">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>所属九项工程：</p>
						<p>
							<el-select v-model="form.belongType" clearable placeholder="全部" style="width:100%" :disabled="type !='添加'&& type!='update'">
				...
				<div class="item" style="margin-bottom: 34px;">
					<div class="item_item">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>带动类型：</p>
						<p>
					...
					<div class="item_item">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>带动贫困人口增收方式：</p>
						<p>
							<el-select v-model="form.incomeType" @change="handleIncomeType" clearable style="width:100%" placeholder="全部" :disabled="type !='添加'&& type!='update'">
					...
					<div class="item_item" v-if="form.incomeType == '其他间接带动方式'">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>带动方式描述：</p>
						<p>
							<p>
								<el-input v-model="form.assistTypeDescribe" placeholder="补填带动方式描述" :maxLength="50" clearable :disabled="type !='添加'&& type!='update'">
								</el-input>
							</p>
						</p>
					</div>
					...
					<div class="item_item">
						...
						<p>
							<el-date-picker v-model="form.startTime" @change="changeStartTime" :picker-options="disDate" value-format="yyyy-MM" type="month" placeholder="选择日期" style="width:100%" :disabled="type !='添加'&& type!='update'">
							</el-date-picker>
						</p>
					</div>
					<div class="item_item">
						...
						<p>
							<el-date-picker v-model="form.finishTime" :picker-options="disDate1" value-format="yyyy-MM" type="month" placeholder="选择日期" style="width:100%" :disabled="type !='添加'&& type!='update'">
							</el-date-picker>
						</p>
					</div>
				</div>
				<div class="item" style="margin-bottom: 34px;">
					<div class="item_item">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>收入时间：</p>
						<p>
							<el-date-picker v-model="form.incomeTime" :picker-options="disDate1" value-format="yyyy-MM" type="month" placeholder="选择日期" style="width:100%" :disabled="type !='添加'&& type!='update'">
							</el-date-picker>
						</p>
					</div>
					<div class="item_item">
						<p class="color ft16 mar"><span style="color: red;" v-if="type == '添加' || type == 'update'">*</span>收入金额：</p>
						<p>
							<el-input v-model.number="form.income" @change="moneyChange" :maxLength="6" style="width:100%;" clearable placeholder="收入金额" :disabled="type !='添加'&& type!='update'">
							</el-input>
						</p>
					</div>
					<div class="item_item">
					...
					<div class="item_item">
						...
						<p>
							<el-input v-model="form.remark" clearable :disabled="type !='添加'&& type!='update'">
							</el-input>
						</p>
					</div>
				</div>
				<div class="item1">
					...
					<div class="fuj">
						<div class="img filesLook" :class="type == '添加' || type == 'update' ? 'filesDel' : ''" v-for="(item, i) in fileList">
							<template v-if="imgs.indexOf(item.fileType) != -1">
								<img :src="item.url" />
								<div class="img_tilte ft12">照片描述</div>
								
								<template v-if="type == '添加' || type == 'update'">
						...
						<div class="files" v-if="type == '添加' || type == 'update'">
...
export default {
	data() {
		return {
			...
			disDate1: {
				disabledDate: time => {
					var falg = false;
					if(new Date(this.form.startTime).getTime() > new Date(time).getTime()){
						falg = true
					}
	...
	created() {
		...
		// 如果不修新增或者update的话 就不请求某些api 添加 查看 update 审核
		if(this.type != 'add'){
			this.getData();
		}
	...
	methods: {
		...
		sub(state) {
			...
			fetch.put(oldData.oldData, par, res => {
				if(res.code === 200){
					this.$loading({
						text: res.message
					})
		...
		// 新增选择器禁用
		dis() {
			if(this.type == '添加' || this.type == 'update') { // 新增或者update时
				// 公共禁用
