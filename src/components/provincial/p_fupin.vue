						...
						<div class="filterBox" v-show="isFilter" style="padding: 36px 50px 46px 50px;">
							<el-form :inline="true" :model="filter" class="demo-form-inline">
								...
								<el-form-item label="户籍市州">
									<el-select v-model="filter.cityId" filterable @change="handleCityId" placeholder="全部" clearable>
										<el-option v-for="item in cityOps" :key="item.id" :label="item.orgName" :value="item.id">
										</el-option>
									</el-select>
								</el-form-item>
								<el-form-item label="户籍区县">
									<el-select v-model="filter.countryId" @change="handleCountryId" placeholder="全部" clearable filterable>
										<el-option v-for="item in countryOps" :key="item.id" :label="item.orgName" :value="item.id">
										</el-option>
									</el-select>
								</el-form-item>
								...
								<el-form-item label="项目地址">
									<el-select v-model="filter.projectAddressArr" clearable @change="projectAddressArr" style="width:241px;" placeholder="全部" filterable>
										<el-option v-for="item in cityOps" :key="item.id" :label="item.orgName" :value="item.id">
										</el-option>
									</el-select>
									<el-select v-model="filter.projectAddress" clearable placeholder="全部" style="width:245px;" filterable>
										<el-option v-for="item in projectAddressOps" :key="item.id" :label="item.orgName" :value="item.id">
										</el-option>
									</el-select>
								</el-form-item>
								<el-form-item label="景区名称">
									<el-select v-model="filter.jingqu1" @change="jingqu1" clearable placeholder="全部" style="width:160px;" filterable>
										<el-option v-for="item in cityOps" :key="item.id" :label="item.orgName" :value="item.id">
										</el-option>
									</el-select>
									<el-select v-model="filter.jingqu2" @change="jingqu2" clearable placeholder="全部" style="width:165px;" filterable>
										<el-option v-for="item in jingqu2Ops" :key="item.id" :label="item.orgName" :value="item.id">
										</el-option>
									</el-select>
									<el-select v-model="filter.scenicId" clearable placeholder="全部" style="width:160px;" filterable>
										<el-option v-for="item in jingquOps" :key="item.id" :label="item.orgName" :value="item.id">
										</el-option>
									</el-select>
								</el-form-item>
								...
								<el-form-item label="带动贫困人口增收方式">
									<el-select v-model="filter.incomeType" clearable placeholder="全部" style="width:395px;">
								...
								<el-form-item label="填报员" label-width="76px">
									<el-select v-model="filter.username" :loading="usernameLoading" @focus="remoteMethod" clearable placeholder="全部" filterable>
										<el-option v-for="item in userOps" :key="item.username" :label="item.nickName" :value="item.username">
											<span style="float: left">{{ item.nickName }}</span>
											<span style="float: right; color: #8492a6; font-size: 13px">{{item.city + ' ' + item.country}}</span>
										</el-option>
									</el-select>
								</el-form-item>
								
								<el-form-item label="收入时间" label-width="76px">
									<el-date-picker v-model="filter.incomeTime" type="year" placeholder="全部" format="yyyy年"  value-format="yyyy">
									</el-date-picker>
								</el-form-item>
							...
							<div class="filter_btn">
								...
								<!--<div class="btn btn2">发送短信</div>-->
								<div class="btn btn3 btn-disabled">批量导出</div>
							</div>
						</div>
					</transition>
				</div>
	...
	methods: {
		...
		getData() {
			let par = {
				...
				projectAddress: this.filter.projectAddress || this.filter.projectAddressArr,
		...
		handleCityId(v) {
			...
			let par = {
				orgId: this.filter.cityId
			}
			fetch.get(allApi.getDivsion, par, res => {
				if (res.code === 200) {
					this.countryOps = res.data
				}
			})
		},
		handleCountryId(v) {
		...
		handleTownId(v) {
			...
			fetch.get(clbum.classname, par, res => {
				if (res.code === 200) {
		...
		jingqu1(v) {
			this.filter.jingqu2 = '';
			this.jingqu2Ops = [];
			this.filter.scenicId = '';
			this.jingquOps = [];
			if (!v) { return }
			let par = {
				orgId: this.filter.jingqu1,
			}
			fetch.get(allApi.getDivsion, par, res => {
				if (res.code === 200) {
					this.jingqu2Ops = res.data
				}
			})
		},
		jingqu2(v) {
			...
			if (!v) { return }
			let par = {
				orgId: this.filter.jingqu2,
				orgLevel: 6
			}
			fetch.get(allApi.getDivsion, par, res => {

				if (res.code === 200) {
					this.jingquOps = res.data
				}
			})
		},
		// 项目地址切换
		projectAddressArr(v) {
			this.filter.projectAddress = '';
			this.projectAddressOps = [];
			if (!v) { return }
			let par = {
				orgId: this.filter.projectAddressArr,
			}
			fetch.get(allApi.getDivsion, par, res => {
				if (res.code === 200) {
					this.projectAddressOps = res.data
				}
			})
		},
		...
		remoteMethod() {
			...
			fetch.get(fupin.reportMember, '', res => {
				if (res.code === 200) {
					this.userOps = res.data
				}
