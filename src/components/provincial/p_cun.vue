			...
			<div class="main_content">
				<div class="filter" @mouseenter='isFilter=true' @click.stop='isFilter=true'>
					...
					<el-form :inline="true" :model="filter" class="demo-form-inline">
						<el-form-item label="归属市州">
							<el-select v-model="filter.city" filterable placeholder="全部" @change="handleCountryId" clearable>
								<el-option v-for="item in countyOps" :key="item.id" :label="item.orgName" :value="item.id">
								</el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="归属区县">
							<el-select v-model="filter.countryId" filterable placeholder="全部" clearable @change="handleTownId">
								<el-option v-for="item in villageOps" :key="item.id" :label="item.orgName" :value="item.id">
								</el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="归属乡镇">
							<el-select v-model="filter.townId" filterable placeholder="全部" clearable>
								<el-option v-for="item in villageOpss" :key="item.id" :label="item.orgName" :value="item.id">
								</el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="村名">
							<el-input v-model="filter.user" placeholder="村名" clearable>
				...
				<div class="table">
					<el-table ref="table" :data="tableData" tooltip-effect="dark" style="width: 99%;" height="100%" header-cell-class-name="tableTou" cell-class-name="tableRow" @selection-change="handleSelectionChange">
						...
						<el-table-column prop="villageSecretaryPhone" align="center" label="村支书联系电话" show-overflow-tooltip>
						</el-table-column>
						<el-table-column prop="residentSecretaryName" align="center" label="驻村第一书记姓名" show-overflow-tooltip>
						</el-table-column>
						<el-table-column prop="residentSecretaryPhone" align="center" label="驻村第一书记联系电话" width="150" show-overflow-tooltip>
						</el-table-column>
						<el-table-column prop="updateTime" align="center" label="更新日期" width="110" show-overflow-tooltip>
